...
void Time::addMilliseconds(int32_t milliseconds)
{
	...
	if (milliseconds >= 0)
	{
		...
		m_millisecond = (m_millisecond + milliseconds) % 1000;
	}
	else
	{
		if ((static_cast<int32_t>(millisecond()) + milliseconds) >= 0)
		{
			m_millisecond = (milliseconds + m_millisecond) % 1000;
		}
		else
		{
			addsec = ((milliseconds + millisecond()) / 1000) - 1;
			m_millisecond = (milliseconds + m_millisecond + 1000) % 1000;
		}
	}
...
void Time::addSeconds(int32_t seconds)
{
	...
	if (seconds >= 0)
	{
		...
		m_second = (m_second + seconds) % 60;
	}
	else
	{
		if ((static_cast<int32_t>(second()) + seconds) >= 0)
		{
			m_second = (seconds + m_second) % 60;
		}
		else
		{
			addmin = ((seconds + second()) / 60) - 1;
			m_second = ((seconds + m_second) % 60) + 60;
		}
	}
...
void Time::addMinutes(int32_t minutes)
{
	...
	if (minutes >= 0)
	{
		...
		m_minute = (m_minute + minutes) % 60;
	}
	else
	{
		if ((static_cast<int32_t>(minute()) + minutes) >= 0)
		{
			m_minute = (minutes + m_minute) % 60;
		}
		else
		{
			addhours = ((minutes + minute()) / 60) - 1;
			m_minute = (minutes + m_minute % 60) + 60;
		}
	}
...
{
	return static_cast<int32_t>(millisecondsSinceStartOfDay()) -
			static_cast<int32_t>(subtract.millisecondsSinceStartOfDay());
}

Time& Time::operator=(const Time& timeToSet)
{
	setTime(timeToSet.millisecond(), timeToSet.second(),
			timeToSet.minute(), timeToSet.hour());
	return *this;
}

bool Time::operator==(const Time& timeToCompareWith) const
{
	if (hour() == timeToCompareWith.hour() &&
			minute() == timeToCompareWith.minute() &&
			second() == timeToCompareWith.second() &&
...
{
	if (hour() > timeToCompareWith.hour() ||
			(hour() == timeToCompareWith.hour() && minute() > timeToCompareWith.minute()) ||
			(hour() == timeToCompareWith.hour() && minute() == timeToCompareWith.minute() &&
					second() > timeToCompareWith.second()) ||
			(hour() == timeToCompareWith.hour() && minute() == timeToCompareWith.minute() &&
					second() == timeToCompareWith.second() && millisecond() >= timeToCompareWith.millisecond()))
	{
		return false;
	}
	else
