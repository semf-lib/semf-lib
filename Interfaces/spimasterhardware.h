...
#ifndef INTERFACES_SPIMASTERHARDWARE_H_
#define INTERFACES_SPIMASTERHARDWARE_H_

#include <Interfaces/spi.h>
#include <Interfaces/communicationhardwaresynchronousmaster.h>
...
public:
	 ...
	 */
	void setFormat(uint8_t bits, TransmissionMode transmission, WireMode wire);
	/**
	 ...
	 */
	void writeRead(const uint8_t writeData[], uint8_t readBuffer[], size_t size);
	...
	/**
	 * @brief Enables chip select for read or write operation,
	 *        if chip select was set in the past.
	 */
	void enableChipSelect();

	/**
	 * @brief Disables chip select after read or write operation,
	 *        if chip select was set in the past.
	 */
...
protected:
	...
	/**
	 * @brief Forces the hardware to stop a writing process.
	 */
	virtual void stopWriteHardware() = 0;
	/**
	 * @brief Forces the hardware to stop a reading process
	 */
...
#endif  /* INTERFACES_SPIMASTERHARDWARE_H_ */
