...
void Stm32SpiMaster::setFrequency(uint32_t hz)
{
	...
	std::generate(frequencies.begin(), frequencies.end(), [this, prescaler] () mutable
	{
...
void Stm32SpiMaster::writeHardware(const uint8_t data[], size_t dataSize)
{
	...
	HAL_StatusTypeDef state = HAL_SPI_Transmit_IT(m_hwHandle, const_cast<uint8_t*>(data), dataSize);
	if (state != HAL_OK)
	{
		if (state == HAL_ERROR)
		{
...
{
	aborted.reconnect(&writeStopped, &Signal<>::emitSignal);
...
{
	aborted.reconnect(&readStopped, &Signal<>::emitSignal);
	auto state = HAL_SPI_Abort_IT(m_hwHandle);
	if (state != HAL_OK)
	{
		if (state == HAL_ERROR)
...
void Stm32SpiMaster::writeReadNext()
{
	...
	__HAL_UNLOCK(m_hwHandle);
	HAL_StatusTypeDef state = HAL_SPI_TransmitReceive_IT(m_hwHandle,
			const_cast<uint8_t*>(&m_writeData[m_dataIndex]), &m_readData[m_dataIndex], 1);
	if (state != HAL_OK)
	{
		if (state == HAL_ERROR)
		{
