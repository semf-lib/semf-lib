...
#ifndef CORE_LISTS_LINKEDLIST_H_
#define CORE_LISTS_LINKEDLIST_H_
...
{
	return Iterator(m_back->LinkedList::Node::next());  // is nullptr
}

template<class T>
typename LinkedList<T>::ConstIterator LinkedList<T>::end() const
{
	return ConstIterator(m_back->LinkedList::Node::next());  // is nullptr
}
...
void LinkedList<T>::pushBack(T& element)
{
	...
	if (m_size > 1)
	{
		element.LinkedList::Node::setPrevious(m_back);
		m_back->LinkedList::Node::setNext(&element);
		m_back = &element;
	}
	else if (m_size == 1)
	{
		element.LinkedList::Node::setPrevious(m_front);
		m_front->LinkedList::Node::setNext(&element);
		m_back = &element;
	}
	else  // m_size == 0
	{
...
void LinkedList<T>::pushFront(T& element)
{
	...
	{
		element.LinkedList::Node::setNext(m_back);
		m_back->LinkedList::Node::setPrevious(&element);
		m_front = &element;
	}
	else  // m_size == 0
	{
...
void LinkedList<T>::popBack()
{
	...
	{
		m_back = m_back->LinkedList::Node::previous();
		m_size--;
	}
}
...
#endif /* CORE_LISTS_LINKEDLIST_H_ */
