...
#ifndef CORE_QUEUES_LINKEDQUEUE_H_
#define CORE_QUEUES_LINKEDQUEUE_H_
...
public:
	...
	public:
		...
		{
			Node* me = this;
			return queue.end() != std::find_if(queue.begin(), queue.end(), [&me](T& node)
			{
				return &node == me;
			});
		}
...
{
	return Iterator(m_back->LinkedQueue::Node::next());  // is nullptr
}

template<class T>
typename LinkedQueue<T>::ConstIterator LinkedQueue<T>::end() const
{
	return ConstIterator(m_back->LinkedQueue::Node::next());  // is nullptr
}
...
#endif /* CORE_QUEUES_LINKEDQUEUE_H_ */
