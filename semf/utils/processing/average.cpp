...
{
	uint32_t binPlacesDivider = 0;  // for counting the binary places of the divider
	T mask = 0;
	size_t divideHelper = 0;  // for saving the intermediate steps by the division
	T result = 0;

	// counting the binaryplaces of the divider
	for (uint32_t i = 0; i < static_cast<uint32_t>(sizeof(divider)) * 8; i++)
	{
		if (!(divider >> i))
		{
			binPlacesDivider = i;
	...
	for (uint32_t i = 0; i < binPlacesDivider; i++)
	{
		mask = mask << 1;
		mask += 1;
	}

	divideHelper = highValue >> static_cast<T>(((sizeof(std::size_t) * 8) - binPlacesDivider) & mask);
	...
	for (int i = ((sizeof(T) + sizeof(std::size_t)) * 8) - binPlacesDivider; i >= 0; i--)
	{
		...
		if (i > static_cast<int>(sizeof(T)) * 8)
			divideHelper = divideHelper + static_cast<T>((highValue >> (i - (sizeof(T) * 8) - 1)) & 1);
		else
			divideHelper = divideHelper + static_cast<T>((lowValue >> (i - 1)) & 1);
	}
	return result;
}
} /* namespace semf */
