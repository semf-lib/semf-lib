...
#include <semf/utils/system/date.h>
...
bool Date::setDate(uint8_t day, Month month, uint16_t year)
{
	...
	{
		SEMF_ERROR("invalid parameter in setDate: y %u; m %u; d %u", year, static_cast<uint8_t>(month), day);
	}
	return isvalid;
}
...
bool Date::setDay(uint8_t day)
{
	...
	{
		SEMF_ERROR("invalid parameter in setDay: y %u; m %u; d %u", year(), static_cast<uint8_t>(month()), day);
	}
	return isvalid;
}
...
void Date::addDays(int32_t days)
{
	...
	m_day = static_cast<uint8_t>(m_day + days);
...
void Date::addYears(int32_t years)
{
	...
	{
		m_year = static_cast<uint16_t>(m_year + years);
	}
	else
	{
		m_year = 0;
...
{
	if (year() == dateToCompareWith.year() && month() == dateToCompareWith.month() && day() == dateToCompareWith.day())
	{
		return true;
	}
	else
...
bool Date::operator<(const Date& dateToCompareWith) const
{
	if (year() < dateToCompareWith.year() || (year() == dateToCompareWith.year() && month() < dateToCompareWith.month()) ||
		(year() == dateToCompareWith.year() && month() == dateToCompareWith.month() && day() < dateToCompareWith.day()))
	{
		return true;
	}
	else
