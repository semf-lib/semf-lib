...
namespace semf
{
DateTime::DateTime(uint16_t millisecond, uint8_t second, uint8_t minute, uint8_t hour, uint8_t day, Month month, uint16_t year)
{
...
bool DateTime::setDateTime(uint16_t millisecond, uint8_t second, uint8_t minute, uint8_t hour, uint8_t day, Month month, uint16_t year)
{
	...
	{
		SEMF_ERROR("invalid parameter in setDateTime: y %u; m %u; d %u; h %u; m %u; s %u; mi %u", year, static_cast<uint8_t>(month), day, hour, minute, second,
				   millisecond);
		return false;
	}
}
...
{
	return setDateTime(copyFromTime.millisecond(), copyFromTime.second(), copyFromTime.minute(), copyFromTime.hour(), copyFromDate.day(), copyFromDate.month(),
					   copyFromDate.year());
}
...
void DateTime::setDateTime(uint64_t unixTimeInSeconds)
{
	...
	// Second
	second = static_cast<uint8_t>(unixTimeInSeconds % 60);
	unixTimeInSeconds /= 60;
	// Minute
	minute = static_cast<uint8_t>(unixTimeInSeconds % 60);
	unixTimeInSeconds /= 60;
	// Hour
	hour = static_cast<uint8_t>(unixTimeInSeconds % 24);
	...
	// Day
	day = static_cast<uint16_t>(unixTimeInSeconds + 1);
...
uint64_t DateTime::unixTimeInSeconds() const
{
	...
	SEMF_INFO("unixTimeInSeconds %u out of y %u; m %u; d %u; h %u; m %m; s %u", ts, year, mon, day, hrs, min, sec);
	return ts;
}
...
void DateTime::addHours(int32_t hours)
{
	...
	if (hours >= 0)
	{
		...
		setHour(static_cast<uint8_t>((hour() + hours) % 24));
	}
	else
	{
		...
		else
		{
			...
			setHour(static_cast<uint8_t>((24 + hour() + hours) % 24));
		}
	}
...
{
	if (Date::operator==(static_cast<Date>(other)) && Time::operator==(static_cast<Time>(other)))
	{
		return true;
	}
	else
...
bool DateTime::operator<(const DateTime& other) const
{
	if (Date::operator<(static_cast<Date>(other)) || (Date::operator==(static_cast<Date>(other)) && Time::operator<(static_cast<Time>(other))))
	{
		return true;
	}
	else
