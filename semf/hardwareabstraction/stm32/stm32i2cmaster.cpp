...
{
	queue()->push(*this);
	canceled.connect(m_canceledSlot);
}
...
void Stm32I2cMaster::stopWrite()
{
	...
	m_canceledSlot.setObject(writeStopped);
...
void Stm32I2cMaster::stopRead()
{
	...
	m_canceledSlot.setObject(readStopped);
	HAL_StatusTypeDef state = HAL_I2C_Master_Abort_IT(m_hwHandle, address());
	if (state != HAL_OK)
	{
		if (state == HAL_ERROR)
...
void Stm32I2cMaster::checkAddress(uint8_t address)
{
	...
	setAddress(address);
	setBusy(true);
	dataAvailable.connect(m_onAddressCheckedSlot);
...
void Stm32I2cMaster::readHardware(uint8_t buffer[], size_t bufferSize)
{
	...
	{
		state = HAL_I2C_Master_Receive_IT(m_hwHandle, static_cast<uint16_t>(address() << 1), buffer, static_cast<uint16_t>(bufferSize));
	}
	else
	{
		state =
			HAL_I2C_Master_Seq_Receive_IT(m_hwHandle, static_cast<uint16_t>(address() << 1), buffer, static_cast<uint16_t>(bufferSize), hardwareFrame(frame()));
	}
	if (state != HAL_OK)
	{
		if (state == HAL_ERROR)
...
void Stm32I2cMaster::writeHardware(const uint8_t data[], size_t dataSize)
{
	...
	if (frame() == Frame::FirstAndLast)
	{
		state = HAL_I2C_Master_Transmit_IT(m_hwHandle, static_cast<uint16_t>(address() << 1), const_cast<uint8_t*>(data), static_cast<uint16_t>(dataSize));
	}
	else
	{
		state = HAL_I2C_Master_Seq_Transmit_IT(m_hwHandle, static_cast<uint16_t>(address() << 1), const_cast<uint8_t*>(data), static_cast<uint16_t>(dataSize),
											   hardwareFrame(frame()));
	}
...
void Stm32I2cMaster::onAddressChecked()
{
	dataAvailable.disconnect(m_onAddressCheckedSlot);
	setBusy(false);
	addressFound();
}
} /* namespace semf */
