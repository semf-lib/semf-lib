...
extern "C" void HAL_CAN_TxMailbox0CompleteCallback(CAN_HandleTypeDef* can)
{
	semf::Stm32Can::systemIsrWritten(*can, 0);
}
extern "C" void HAL_CAN_TxMailbox1CompleteCallback(CAN_HandleTypeDef* can)
{
	semf::Stm32Can::systemIsrWritten(*can, 1);
}
extern "C" void HAL_CAN_TxMailbox2CompleteCallback(CAN_HandleTypeDef* can)
{
	semf::Stm32Can::systemIsrWritten(*can, 3);
}
extern "C" void HAL_CAN_ErrorCallback(CAN_HandleTypeDef* can)
{
	semf::Stm32Can::systemIsrError(*can);
}
extern "C" void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef* can)
{
	semf::Stm32Can::systemIsrRead(*can, 0);
}
extern "C" void HAL_CAN_RxFifo1MsgPendingCallback(CAN_HandleTypeDef* can)
{
...
void Stm32Can::isrRead(CAN_HandleTypeDef& can, uint8_t fifoId)
{
	...
	HAL_StatusTypeDef state = HAL_CAN_GetRxMessage(m_hwHandle, fifoId == 0 ? CAN_RX_FIFO0 : CAN_RX_FIFO1, &m_rxHeader, m_readData /*[m_readDataIndex]*/);
	if (state != HAL_OK)
	{
		if (state == HAL_ERROR)
		{
...
void Stm32Can::writeHardware(const uint8_t data[], size_t dataSize)
{
	...
	if (dataSize > 8)
	{
		SEMF_WARNING("The given dataSize is greater than 8, but this STM32-CAN implementation doesn't support CAN FD. The value is changed to 8");
	}

	size_t writeSize = std::min(dataSize, std::size_t{8});

	m_writeData = data;
	m_writeDataIndex = 0;
	m_writeDataSize = writeSize;

	m_txHeader.DLC = writeSize;
...
void Stm32Can::start()
{
	...
	HAL_StatusTypeDef notificationState =
		HAL_CAN_ActivateNotification(m_hwHandle, CAN_IT_RX_FIFO0_MSG_PENDING | CAN_IT_RX_FIFO1_MSG_PENDING | CAN_IT_TX_MAILBOX_EMPTY);
