...
#ifndef PROCESSING_CRCSOFTWARE_H_
#define PROCESSING_CRCSOFTWARE_H_
...
public:
	 ...
	 */
	explicit CrcSoftware(T polynomial = DEFAULT_POLYNOMIAL, T initValue = DEFAULT_INITVALUE,
			T finalXor = DEFAULT_FINALXOR);

	void reset();
	const uint8_t* accumulate(const uint8_t data[], size_t dataSize);
	const uint8_t* calculate(const uint8_t data[], size_t dataSize);
	bool isEqual(const uint8_t data[]);
...
template<typename T, T DEFAULT_POLYNOMIAL, T DEFAULT_INITVALUE, T DEFAULT_FINALXOR>
CrcSoftware<T, DEFAULT_POLYNOMIAL, DEFAULT_INITVALUE, DEFAULT_FINALXOR>::CrcSoftware(
		T polynomial, T initValue, T finalXor)
...
template<typename T, T DEFAULT_POLYNOMIAL, T DEFAULT_INITVALUE, T DEFAULT_FINALXOR>
const uint8_t* CrcSoftware<T, DEFAULT_POLYNOMIAL, DEFAULT_INITVALUE, DEFAULT_FINALXOR>::accumulate(
...
template<typename T, T DEFAULT_POLYNOMIAL, T DEFAULT_INITVALUE, T DEFAULT_FINALXOR>
const uint8_t* CrcSoftware<T, DEFAULT_POLYNOMIAL, DEFAULT_INITVALUE, DEFAULT_FINALXOR>::calculate(
		const uint8_t data[], size_t dataSize)
{
	m_crc = m_initValue ^ m_finalXor;
...
#endif  /* PROCESSING_CRCSOFTWARE_H_ */
