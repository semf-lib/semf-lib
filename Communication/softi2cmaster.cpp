...
#include <Communication/softi2cmaster.h>
#include <Core/debug.h>

namespace semf
{
SoftI2cMaster::SoftI2cMaster(Gpio& scl, Gpio& sda, Timer& timer)
	:m_scl(scl),
	 m_sda(sda),
	 m_timer(timer)
{
...
void SoftI2cMaster::writeHardware(const uint8_t data[], size_t size)
{
	...
	if (frame() == CommunicationHardware::Frame::First ||
			frame() == CommunicationHardware::Frame::FirstAndLast)
	{
...
void SoftI2cMaster::readHardware(uint8_t buffer[], size_t bufferSize)
{
	...
	if (frame() == CommunicationHardware::Frame::First ||
			frame() == CommunicationHardware::Frame::FirstAndLast)
	{
		m_activeByte = (address() << 1) | 0x01;
		m_writingAddress = true;
		startConditionResetSda();
	}
	else if (m_lastOperationWasWrite &&
			(m_lastFrame == CommunicationHardware::Frame::First ||
					m_lastFrame == CommunicationHardware::Frame::Next))
	{
		m_activeByte = (address() << 1) | 0x01;
		m_writingAddress = true;
...
void SoftI2cMaster::resartConditionSetSdaResetScl()
{
	...
	m_sda.set();
	m_scl.reset();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::resartConditionSetScl);
}

void SoftI2cMaster::resartConditionSetScl()
{
	m_scl.set();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::startConditionResetSda);
}
...
void SoftI2cMaster::startConditionResetSda()
{
	...
	m_sda.reset();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::startConditionResetScl);
}
...
void SoftI2cMaster::writeByteSetSdaDataBit()
{
	...
	if (m_activeByte & (1 << m_bitIndex))
		...
		m_sda.reset();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::writeByteSetScl);
}

void SoftI2cMaster::writeByteSetScl()
{
	m_scl.set();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::writeByteResetScl);
}
...
void SoftI2cMaster::readByteReadSdaDataBit()
{
	...
	m_timer.timeout.reconnect(this, &SoftI2cMaster::readByteSetScl);
}

void SoftI2cMaster::readByteSetScl()
{
	m_scl.set();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::readByteResetScl);
}
...
void SoftI2cMaster::checkAcknowledgeSetSdaInput()
{
	m_sda.set();
	m_sda.setDirection(Gpio::Direction::Input);
	m_timer.timeout.reconnect(this, &SoftI2cMaster::checkAcknowledgeSetScl);
}
...
void SoftI2cMaster::checkAcknowledgeSetScl()
{
	...
	m_timer.timeout.reconnect(this, &SoftI2cMaster::checkAcknowledgeResetScl);
}
...
void SoftI2cMaster::setAcknowledgeSetSdaOutput()
{
	...
	if (m_dataIndex + 1 < m_dataSize)
		m_sda.reset();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::setAcknowledgeSetScl);
}

void SoftI2cMaster::setAcknowledgeSetScl()
{
	m_scl.set();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::setAcknowledgeResetScl);
}
...
void SoftI2cMaster::finishAcknowledgeWriteOperation()
{
	...
	{
		if (frame() == CommunicationHardware::Frame::Last ||
				frame() == CommunicationHardware::Frame::FirstAndLast)
		{
			stopConditionResetSda();
		}
...
void SoftI2cMaster::stopConditionResetSda()
{
	...
	m_sda.reset();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::stopConditionSetScl);
}
...
void SoftI2cMaster::stopConditionSetScl()
{
	m_scl.reset();
	m_scl.set();
	m_timer.timeout.reconnect(this, &SoftI2cMaster::stopConditionSetSda);
}
