...
namespace semf
{
LedDimming::LedDimming(PwmOut& dimmPwmOut, TimeBase& timebase)
...
void LedDimming::setSine(unsigned int periodInTimeBaseTicks)
{
	...
	if (m_mode != Mode::Sine)
	{
		...
		m_timer.timeout.reconnect(this, &LedDimming::nextSineValue);
		m_timer.start();
		m_dimmPwm.setEnable();
	}
}
...
void LedDimming::setTriangle(unsigned int periodInTimeBaseTicks)
{
	...
	if (m_mode != Mode::Triangule)
	{
		...
		m_timer.reset();
		m_timer.setInterval(1);
		m_timer.timeout.reconnect(this, &LedDimming::nextTriangleValue);
		m_timer.start();
		m_dimmPwm.setEnable();
	}
}
...
void LedDimming::startOnTimeInBlinkingMode()
{
	...
	m_timer.timeout.reconnect(this, &LedDimming::startOffTimeInBlinkingMode);
...
void LedDimming::startOffTimeInBlinkingMode()
{
	...
	m_timer.timeout.reconnect(this, &LedDimming::startOnTimeInBlinkingMode);
...
void LedDimming::nextSineValue()
{
	...
	m_dimmPwm.setPermille(m_minBrightness +
			static_cast<uint16_t>(static_cast<double>(m_maxBrightness-m_minBrightness)/1000*tmpBrightness));
